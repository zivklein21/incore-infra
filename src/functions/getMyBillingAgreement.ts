...
export async function handler(
  ...
  const [agreementRes, profileRes] = await Promise.all([
    ...
    ddb.send(new GetCommand({ TableName: TABLE_NAME, Key: { PK: `MEMBER#${uid}`, SK: 'PROFILE' }, ConsistentRead: true })),
  ]);

  const agreement = (agreementRes.Items ?? [])[0] as HypBillingAgreementItem | undefined;
  const forceShowPaymentButton = (profileRes.Item as MemberProfileItem | undefined)?.admin?.forceShowPaymentButton === true;

  if (!agreement) return json(200, { hasAgreement: false, forceShowPaymentButton });
